...
public class MainActivity extends Activity {
    ...
    private ContentFrameLayout cf; private ActionBarDrawerToggle mDrawerToggle;
    private DrawerLayout mDrawerLayout;
    private ListView leftList,rightList;
    private SimpleAdapter leftListAdapter,rightListAdapter;
    private ArrayList leftListItem,rightListItem;
    private SeekBar seekBar; private TableLayout backLayout;
    private int valRed=0,valBlue=0,valGreen=0; private int setRadix = 10;
    private SQLiteDatabase sqLiteDatabase;
   private void addItem(int id,String title,ArrayList al) {HashMap map=new HashMap(); map.put("image", id); map.put("title",title); al.add(map); }
    private void startConver(){
        if(inputText.getText().length()<1) return;
        try { binText.setText(new BigInteger(inputText.getText().toString(), setRadix).toString(2).toUpperCase());
    ...
    private void setButtonEnabled(int radix){
        inputText.setText(""); initOutputResult();
        if(radix == 2){
            for(int i = 0 ;i<2;i++){
                button[i].setEnabled(true);
                button[i].setTextColor(Color.WHITE); }
            for(int i = 2 ;i<16;i++){
                button[i].setEnabled(false); button[i].setTextColor(Color.GRAY); } }
        if(radix==10){
            for(int i = 0 ;i<10;i++){
                button[i].setEnabled(true);
                button[i].setTextColor(Color.WHITE); }
    ...
    { super.onCreate(savedInstanceState);
        ...
        Animation buttonScale = new ScaleAnimation(0.0f,1f, 0.0f, 1f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
        ...
        try {
            ...
            backLayout.setBackgroundColor(Color.rgb(Integer.valueOf(sharedata.getString("red", null)), Integer.valueOf(sharedata.getString("green", null)),Integer.valueOf(sharedata.getString("blue", null)))); }
        ...
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onStopTrackingTouch(SeekBar seekBar) {
                ...
                sharedata.commit(); } });
        //设置ListView
        leftListItem = new ArrayList<>();
        rightListItem = new ArrayList<>();
        ...
        /*leftListAdapter = new SimpleAdapter(this, leftListItem, R.layout.drawer_layout, new String[]{"image","title"}, new int[]{R.id.ItemImage,R.id.ItemTitle});
        rightListAdapter = new SimpleAdapter(this, leftListItem, R.layout.drawer_layout, new String[]{"image","title"}, new int[]{R.id.ItemImage,R.id.ItemTitle});
        */leftList.setAdapter(leftListAdapter);
        rightList.setAdapter(rightListAdapter);
        leftList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView parent, View view, int position, long id) {
                HashMap map = (HashMap) leftListAdapter.getItem(position);
                setButtonEnabled(10); startConver(); } });
        rightList.setOnItemClickListener(new AdapterView.OnItemClickListener()
        { @Override
        public void onItemClick(AdapterView parent, View view, int position, long id) {
            HashMap map = (HashMap) rightListAdapter.getItem(position); startConver(); } });
        binText.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                initOutputResult();setButtonEnabled(2);
                setRadix = 2;
        ...
        decText.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                initOutputResult(); setButtonEnabled(10);
                setRadix=10;
                binText.setTextColor(Color.parseColor("#ffffffff"));
                octText.setTextColor(Color.parseColor("#ffffffff"));
        ...
        binText.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                openAlertDialog(binText.getText()).show(); return true; } });
        ...
        inputText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if(inputText.getText().length()>0) { startConver(); }
                else initOutputResult(); }
            @Override
            public void afterTextChanged(Editable s) { } });
        numClickListener nc = new numClickListener();
        //设置监听
        for(Button ac : button){ ac.setOnClickListener(nc); } setButtonEnabled(10); }
    ...
    class numClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            Animation buttonScale = new ScaleAnimation(1f,0.8f, 1.0f, 0.8f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
    ...
    public void onBackPressed() {
        ...
        if(isClosing==false){
            ...
            Animation buttonScale = new ScaleAnimation(1f,0.0f, 1.0f, 0.0f, Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF, 0.5f);
            buttonScale.setDuration(1000); for(int i =0;i<18;i++){ button[i].startAnimation(buttonScale); }
            ...
            buttonScale.setAnimationListener(new Animation.AnimationListener() {
                ...
                public void onAnimationStart(Animation animation) { }
                @Override
                public void onAnimationEnd(Animation animation) { System.exit(0); }
                @Override
                public void onAnimationRepeat(Animation animation) { } }); } }}
